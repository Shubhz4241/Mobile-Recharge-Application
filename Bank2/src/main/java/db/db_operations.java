...
public class db_operations {
	...
	public List<mobile_recharge> display_db(int accno) {
		...
		try {
			PreparedStatement pstate=con.prepareStatement("select * from bank_recharge "
					+ "where acc_no=?");
			pstate.setInt(1,accno);
			ResultSet rs=pstate.executeQuery();
			while(rs.next())
			{
				mobile_recharge mr=new mobile_recharge(rs.getInt(1),rs.getString(2),rs.getString(3)
